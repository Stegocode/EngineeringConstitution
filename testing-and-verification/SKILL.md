...
## Test the claim, not just the happy path

For every constraint a function's docstring or error message states, write a test
that violates *exactly that constraint* and asserts the failure. If the docstring
says "no leading/trailing dots," feed `"a."` and `".a"` and assert it raises —
testing only `""` and `"."` leaves the stated rule unproven. When you fix one
member of an input family, add a test for **each** sibling member at the same
time (see the safety skill's input-family rule). An untested claim is a guess
wearing a docstring.
